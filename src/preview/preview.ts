...
import { ImageInfo, DjDocumentRenderer } from './documentRenderer';
...
interface DjotPreviewDelegate {
	getTitle?(resource: vscode.Uri): string;
	getAdditionalState(): {};
	openPreviewLinkToDjotFile(djotLink: vscode.Uri, fragment: string | undefined): void;
}

class DjotPreview extends Disposable implements WebviewResourceProvider {
	...
	constructor(
		...
		private readonly _delegate: DjotPreviewDelegate,
		...
		this._register(this._webviewPanel.webview.onDidReceiveMessage((e: FromWebviewMessage.Type) => {
			...
			switch (e.type) {
				...
				case 'showPreviewSecuritySelector':
					vscode.commands.executeCommand('djot.showPreviewSecuritySelector', e.source);
					break;

				case 'previewStyleLoadError':
					vscode.window.showWarningMessage(
						vscode.l10n.t("Could not load 'djot.styles': {0}", e.unloadedStyles.join(', ')));
					break;
			}
		}));
	...
	*/
	...
	public scrollTo(topLine: number) {
		...
		this._logger.verbose('DjotPreview', 'updateForView', { djotFile: this._resource });
		this._line = topLine;
		this.postMessage({
			type: 'updateView',
			line: topLine,
	...
	private async _onDidClickPreview(line: number): Promise<void> {
		...
		await vscode.commands.executeCommand('djot.showSource');
	...
	private _updateImageWatchers(srcs: Set<string>) {
		...
		for (const src of srcs) {
			...
			if (uri && !DjotPreview._unwatchedImageSchemes.has(uri.scheme) && !this._fileWatchersBySrc.has(src)) {
...
export interface IManagedDjotPreview {
...
export class StaticDjotPreview extends Disposable implements IManagedDjotPreview {

	public static readonly customEditorViewType = 'vscode.djot.preview.editor';

	public static revive(
		resource: vscode.Uri,
		webview: vscode.WebviewPanel,
		contentProvider: DjDocumentRenderer,
	...
	private constructor(
		...
		this._preview = this._register(new DjotPreview(this._webviewPanel, resource, topScrollLocation, {
			getAdditionalState: () => { return {}; },
			openPreviewLinkToDjotFile: (djotLink, fragment) => {
				return vscode.commands.executeCommand('vscode.openWith', djotLink.with({
					fragment
...
export class DynamicDjotPreview extends Disposable implements IManagedDjotPreview {

	public static readonly viewType = 'djot.preview';

	private readonly _resourceColumn: vscode.ViewColumn;
	private _locked: boolean;

	private readonly _webviewPanel: vscode.WebviewPanel;
	private _preview: DjotPreview;
	...
	public static create(
		...
		const webview = vscode.window.createWebviewPanel(
			...
			DynamicDjotPreview._getPreviewTitle(input.resource, input.locked),
	...
	private constructor(
		...
		this._register(vscode.window.onDidChangeActiveTextEditor(editor => {
			...
			if (isDjotFile(editor.document) && !this._locked && !this._preview.isPreviewOf(editor.document.uri)) {
	...
	public toggleLock() {
		...
		this._webviewPanel.title = DynamicDjotPreview._getPreviewTitle(this._preview.resource, this._locked);
	}
	...
	public matches(otherPreview: DynamicDjotPreview): boolean {
		return this.matchesResource(otherPreview._preview.resource, otherPreview.position, otherPreview._locked);
	}

	private _createPreview(resource: vscode.Uri, startingScroll?: StartingScrollLocation): DjotPreview {
		return new DjotPreview(this._webviewPanel, resource, startingScroll, {
			getTitle: (resource) => DynamicDjotPreview._getPreviewTitle(resource, this._locked),
			getAdditionalState: () => {
				return {
			...
			openPreviewLinkToDjotFile: (link: vscode.Uri, fragment?: string) => {
