...
import { DjotPreviewManager } from './previewManager';


export const enum DjotPreviewSecurityLevel {
	Strict = 0,
...
export interface ContentSecurityPolicyArbiter {
	getSecurityLevelForResource(resource: vscode.Uri): DjotPreviewSecurityLevel;

	setSecurityLevelForResource(resource: vscode.Uri, level: DjotPreviewSecurityLevel): Thenable<void>;
...
export class ExtensionContentSecurityPolicyArbiter implements ContentSecurityPolicyArbiter {
	...
	public getSecurityLevelForResource(resource: vscode.Uri): DjotPreviewSecurityLevel {
		// Use new security level setting first
		const level = this._globalState.get<DjotPreviewSecurityLevel | undefined>(this._security_level_key + this._getRoot(resource), undefined);
		...
		if (this._globalState.get<boolean>(this._old_trusted_workspace_key + this._getRoot(resource), false)) {
			return DjotPreviewSecurityLevel.AllowScriptsAndAllContent;
		}
		return DjotPreviewSecurityLevel.Strict;
	}

	public setSecurityLevelForResource(resource: vscode.Uri, level: DjotPreviewSecurityLevel): Thenable<void> {
	...
	public shouldAllowSvgsForResource(resource: vscode.Uri) {
		...
		return securityLevel === DjotPreviewSecurityLevel.AllowInsecureContent || securityLevel === DjotPreviewSecurityLevel.AllowScriptsAndAllContent;
	}
...
export class PreviewSecuritySelector {
	...
	public async showSecuritySelectorForResource(resource: vscode.Uri): Promise<void> {
		interface PreviewSecurityPickItem extends vscode.QuickPickItem {
			readonly type: 'moreinfo' | 'toggle' | DjotPreviewSecurityLevel;
		}
		...
		const selection = await vscode.window.showQuickPick<PreviewSecurityPickItem>(
			[
				{
					type: DjotPreviewSecurityLevel.Strict,
					label: markActiveWhen(currentSecurityLevel === DjotPreviewSecurityLevel.Strict) + vscode.l10n.t("Strict"),
					description: vscode.l10n.t("Only load secure content"),
				}, {
					type: DjotPreviewSecurityLevel.AllowInsecureLocalContent,
					label: markActiveWhen(currentSecurityLevel === DjotPreviewSecurityLevel.AllowInsecureLocalContent) + vscode.l10n.t("Allow insecure local content"),
					description: vscode.l10n.t("Enable loading content over http served from localhost"),
				}, {
					type: DjotPreviewSecurityLevel.AllowInsecureContent,
					label: markActiveWhen(currentSecurityLevel === DjotPreviewSecurityLevel.AllowInsecureContent) + vscode.l10n.t("Allow insecure content"),
					description: vscode.l10n.t("Enable loading content over http"),
				}, {
					type: DjotPreviewSecurityLevel.AllowScriptsAndAllContent,
					label: markActiveWhen(currentSecurityLevel === DjotPreviewSecurityLevel.AllowScriptsAndAllContent) + vscode.l10n.t("Disable"),
			...
			placeHolder: vscode.l10n.t("Select security settings for Djot previews in this workspace"),
		});
		if (!selection) {
			return;
		}
